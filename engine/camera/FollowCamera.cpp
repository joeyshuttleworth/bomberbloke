...
FollowCamera::update()
{
  ...
  if (subjectPtr) {
    double distance =
      std::pow(std::pow(subjectPtr->mPosition[0] - mPosition[0], 2) +
                 std::pow(subjectPtr->mPosition[1] - mPosition[1], 2),
               0.5);
    ...
    if (distance > CAMERA_MAX_DISTANCE) {
      ...
      double speed =
        std::sqrt(std::pow(mVelocity[0], 2) + std::pow(mVelocity[1], 2));
      ...
      if (distance < speed / TICK_RATE) {
        ...
        mVelocity[0] =
          (subjectPtr->mPosition[0] - mPosition[0]) / distance * speed;
        mVelocity[1] =
          (subjectPtr->mPosition[1] - mPosition[1]) / distance * speed;
