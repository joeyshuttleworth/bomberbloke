...
double
vectorProduct(dvector vec1, dvector vec2)
{
...
std::array<double, 2>
AbstractCollider::projectOntoAxis(dvector axis)
{
  double minValue = 0.;
  double maxValue = 0.;
  ...
  // projected collider
  std::array<double, 2> retVal = { { (minValue + positionProjection) / axisNorm,
                                     (maxValue + positionProjection) /
                                       axisNorm } };
...
std::array<double, 2>
AbstractCollider::testNormalAxes(AbstractCollider* collider)
{
  ...
  for (int i = 0; i < nVertices; i++) {
    ...
    if (selfProjVal[1] > colliderProjVal[0] &&
        selfProjVal[0] < colliderProjVal[1]) {
      // Intersection with collider projection on the right
      if (selfProjVal[1] - colliderProjVal[0] < minTransMagnitude ||
          minTransMagnitude == -1) {
        minTransMagnitude = selfProjVal[1] - colliderProjVal[0];
        minTransDirection = axis;
      }
    } else if (colliderProjVal[1] > selfProjVal[0] &&
               colliderProjVal[0] < selfProjVal[1]) {
      // Intersection with collider projection on the left
      if (colliderProjVal[1] - selfProjVal[0] < minTransMagnitude ||
          minTransMagnitude == -1) {
