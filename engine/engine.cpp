...
bool handle_collision(actor *a, actor *b){
  ...
  for(int i = 0; i < 2; i++){
    ...
    if((distance[i]=std::abs(a_tmp_pos[i] - b_midpoint[i]))> double(0.5 * (a->mDimmension[i] + b->mDimmension[i]))){
      collision = false;
      break;
    }
  }
  if(collision){
    ...
    if (a->mVelocity[i]<0)
      x_iter=-1;
    a_tmp_pos[i] = a_tmp_pos[i]-x_iter * (1e-8 + (a->mDimmension[i] + b->mDimmension[i]) * 0.5 - std::abs(a_tmp_pos[i] - b_midpoint[i]));
      a->mVelocity[i] = 0;
